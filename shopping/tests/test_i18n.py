...
쇼핑이 코스모스 저장소 안에 있던 동안에는 문구가 코어 카탈로그에 살았고, 코어의
그물이 *"다섯 언어에 답이 있는가"* 를 검사해 주었다. 마켓으로 나온 지금 그 눈은
코어 소스만 본다 — ★여기서 새 문구를 쓰면 아무도 검사하지 않는다★.

그래서 검사도 함께 온다. 이 파일이 지키는 것 셋:

  ① 부르는 문구는 **표에 다 있다**(없으면 그 언어 사용자는 조용히 영어를 본다)
  ② 표에만 있고 **아무도 안 부르는** 문구는 없다(치우지 않으면 계속 는다)
  ③ ★자리표시자를 잃지 않는다★ `{title}`이 빠진 번역은 **정보를 잃은 문장**이다
"""
...
import shopping_i18n
from shopping_i18n import LANGUAGES, TRANSLATIONS

HERE = pathlib.Path(__file__).resolve().parent.parent
SOURCES = ("shopping_scout.py", "shopping_core.py")
...
def test_the_collector_actually_finds_something():
    ...
    assert len(_spoken()) > 25, f"{len(_spoken())}개만 찾았습니다 — 수집이 깨졌습니다"
...
def test_no_translation_loses_a_placeholder():
    """★`{title}`이 빠진 번역은 **정보를 잃은 문장**이다★ 그리고 조용히 그렇게 된다."""
    broken = []
...
def test_a_language_we_do_not_have_falls_back_to_english_not_to_blank():
    """★빈칸은 사용자가 고장으로 읽는다★"""
    assert shopping_i18n.t("Show the next ones", lang="pt") == "Show the next ones"
    assert shopping_i18n.t("Show the next ones", lang="ko") == "다음 보기"


def test_a_word_only_the_core_knows_still_comes_back_translated():
    """★두 벌로 적지 않는다★ 화면 어휘는 코어에 있고, 우리 표에 없으면 거기 묻는다."""
    got = shopping_i18n.t("Unknown", lang="ko")
    assert got and got != "", "코어에 물어보지 않습니다"
