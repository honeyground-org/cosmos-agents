...
항공권 도우미가 코스모스 저장소 안에 있던 동안에는 문구가 코어 카탈로그에 살았고,
...
그래서 검사도 함께 온다. 이 파일이 지키는 것 넷:
  ...
  ③ ★자리표시자를 잃지 않는다★ `{route}`가 빠진 번역은 **정보를 잃은 문장**이다
...
import flights_i18n
from flights_i18n import LANGUAGES, TRANSLATIONS

HERE = pathlib.Path(__file__).resolve().parent.parent
SOURCES = ("flights_scout.py",)
...
def test_the_collector_actually_finds_something():
    ...
    assert len(_spoken()) > 30, f"{len(_spoken())}개만 찾았습니다 — 수집이 깨졌습니다"
...
def test_no_translation_loses_a_placeholder():
    """★`{route}`가 빠진 번역은 **정보를 잃은 문장**이다★ 그리고 조용히 그렇게 된다."""
    broken = []
    for lang in LANGUAGES:
...
def test_a_language_we_do_not_have_falls_back_to_english_not_to_blank():
    """★빈칸은 사용자가 고장으로 읽는다★"""
    assert flights_i18n.t("Nonstop", lang="pt") == "Nonstop"
    assert flights_i18n.t("Nonstop", lang="ko") == "직항"


def test_a_word_only_the_core_knows_still_comes_back_translated():
    """★두 벌로 적지 않는다★ 화면 어휘는 코어에 있고, 우리 표에 없으면 거기 묻는다."""
    got = flights_i18n.t("Unknown", lang="ko")
